...
const NAV: NavItem[] = [
  ...
  },
  {
    ...
    children: [
      ...
      { label: 'Feed Formulas',    to: '/masters/formulas' },
      ...
      `}>
        ...
        <div className="flex items-center justify-between h-14 px-4 border-b border-gray-100 shrink-0">
          {sidebarOpen && (
            <div className="flex items-center gap-2">
              ...
              <span className="font-bold text-sm text-gray-900">Naraendra Farms</span>
            </div>
          )}
        ...
        <div className="px-3 py-3 border-t border-gray-100">
          <div className="flex items-center gap-2">
            ...
            {sidebarOpen && (
              <button onClick={signOut} className="p-1 rounded hover:bg-gray-100 transition-colors">
      ...
      {mobileOpen && (
        <div className="fixed inset-0 z-40 lg:hidden">
          ...
          <aside className="relative w-64 h-full bg-white border-r border-gray-100 flex flex-col">
            ...
            <nav className="flex-1 overflow-y-auto px-3 py-4 flex flex-col gap-1">
              {NAV.map(item => <NavLink key={item.label} item={item} collapsed={false} />)}
            </nav>
          </aside>
        </div>
      )}
