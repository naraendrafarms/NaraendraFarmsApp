...
export const ChatPanel: React.FC = () => {
  ...
  // My groups + last message preview
  ...
  return (
    <>
      <button onClick={() => setOpen(true)} className="relative p-1.5 rounded-lg hover:bg-gray-100" title="Chat">
        <MessageCircle size={18} className="text-gray-500" />
        {anyUnread && <span className="absolute top-0.5 right-0.5 w-2 h-2 rounded-full bg-red-500" />}
      </button>

      {open && (
        <div className="fixed inset-0 z-50 flex justify-end no-print" onClick={() => setOpen(false)}>
          <div className="absolute inset-0 bg-black/30" />
          <div className="relative bg-white w-full max-w-sm h-full shadow-2xl flex flex-col" onClick={e => e.stopPropagation()}>
