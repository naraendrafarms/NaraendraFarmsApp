          ...
          <table className="w-full text-xs">
            <thead>
              ...
              <tr className="bg-gray-100 border-b border-gray-200">
                ...
                <GH span={4} cls="bg-sky-50 text-sky-800">Opening</GH>
                <GH span={5} cls="bg-green-50 text-green-800">Received (Period)</GH>
                <GH span={4} cls="bg-red-50 text-red-800">Wastage (Period)</GH>
                <GH span={4} cls="bg-orange-50 text-orange-800">Sales (Period)</GH>
                <GH span={4} cls="bg-purple-50 text-purple-800">Closing Balance</GH>
              </tr>
              {/* Sub-headers */}
              <tr className="bg-gray-50 border-b border-gray-200 text-gray-500">
                ...
                {/* Opening */}
                <th className="px-1 py-1.5 text-right bg-sky-50/60 font-medium">HE-A</th>
                <th className="px-1 py-1.5 text-right bg-sky-50/60 font-medium">HE-B</th>
                <th className="px-1 py-1.5 text-right bg-sky-50/60 font-medium">HE-C</th>
                <th className="px-1 py-1.5 text-right bg-sky-100/60 font-semibold">JE/TE/BE</th>
                {/* Received */}
                <th className="px-1 py-1.5 text-right bg-green-50/60 font-medium">HE-A</th>
                <th className="px-1 py-1.5 text-right bg-green-50/60 font-medium">HE-B+C</th>
                <th className="px-1 py-1.5 text-right bg-green-50/60 font-medium">JE</th>
                <th className="px-1 py-1.5 text-right bg-green-50/60 font-medium">TE+BE</th>
                <th className="px-1 py-1.5 text-right bg-green-100/60 font-medium">LE</th>
                {/* Wastage */}
                <th className="px-1 py-1.5 text-right bg-red-50/60 font-medium">HE</th>
                <th className="px-1 py-1.5 text-right bg-red-50/60 font-medium">JE</th>
                <th className="px-1 py-1.5 text-right bg-red-50/60 font-medium">TE</th>
                <th className="px-1 py-1.5 text-right bg-red-50/60 font-medium">BE</th>
                {/* Sales */}
                <th className="px-1 py-1.5 text-right bg-orange-50/60 font-medium">HE-A</th>
                <th className="px-1 py-1.5 text-right bg-orange-50/60 font-medium">HE-B+C</th>
                <th className="px-1 py-1.5 text-right bg-orange-50/60 font-medium">JE</th>
                <th className="px-1 py-1.5 text-right bg-orange-50/60 font-medium">TE+BE</th>
                {/* Closing */}
                <th className="px-1 py-1.5 text-right bg-purple-50/60 font-semibold">HE-A</th>
                <th className="px-1 py-1.5 text-right bg-purple-50/60 font-semibold">HE-B+C</th>
                <th className="px-1 py-1.5 text-right bg-purple-50/60 font-semibold">JE</th>
                <th className="px-1 py-1.5 text-right bg-purple-50/60 font-semibold">TE+BE</th>
              </tr>
            </thead>
            <tbody>
              {stockRows.map((r, idx) => (
                <tr key={r.id} className={`border-b border-gray-100 hover:bg-gray-50 ${r.hasNegative ? 'bg-red-50' : idx % 2 === 0 ? 'bg-white' : 'bg-gray-50/40'}`}>
                  ...
                  {/* Opening HE */}
                  <td className="px-1 py-1.5 text-right text-sky-700 bg-sky-50/30">{fmt(r.opA)}</td>
                  <td className="px-1 py-1.5 text-right text-sky-700 bg-sky-50/30">{fmt(r.opB + r.opC)}</td>
                  <td className="px-1 py-1.5 text-right text-sky-700 bg-sky-50/30 font-medium">{fmt(r.totalOpenHE)}</td>
                  {/* Opening NHE */}
                  <td className="px-1 py-1.5 text-right text-sky-700 bg-sky-100/30 font-medium">{fmt(r.totalOpenNHE)}</td>
                  {/* Received HE */}
                  <td className="px-1 py-1.5 text-right text-green-700 bg-green-50/30">{fmt(r.recA)}</td>
                  <td className="px-1 py-1.5 text-right text-green-700 bg-green-50/30">{fmt(r.recB + r.recC)}</td>
                  {/* Received NHE */}
                  <td className="px-1 py-1.5 text-right text-green-700 bg-green-50/30">{fmt(r.recJE)}</td>
                  <td className="px-1 py-1.5 text-right text-green-700 bg-green-50/30">{fmt(r.recTE + r.recBE)}</td>
                  <td className="px-1 py-1.5 text-right text-green-600 bg-green-100/30">{fmt(r.recLE)}</td>
                  {/* Wastage */}
                  <td className="px-1 py-1.5 text-right text-red-600 bg-red-50/30">{fmt(r.wsHE)}</td>
                  <td className="px-1 py-1.5 text-right text-red-600 bg-red-50/30">{fmt(r.wsJE)}</td>
                  <td className="px-1 py-1.5 text-right text-red-600 bg-red-50/30">{fmt(r.wsTE)}</td>
                  <td className="px-1 py-1.5 text-right text-red-600 bg-red-50/30">{fmt(r.wsBE)}</td>
                  {/* Sales HE */}
                  <td className="px-1 py-1.5 text-right text-orange-700 bg-orange-50/30">{fmt(r.slA)}</td>
                  <td className="px-1 py-1.5 text-right text-orange-700 bg-orange-50/30">{fmt(r.slB + r.slC)}</td>
                  {/* Sales NHE */}
                  <td className="px-1 py-1.5 text-right text-orange-700 bg-orange-50/30">{fmt(r.slJE)}</td>
                  <td className="px-1 py-1.5 text-right text-orange-700 bg-orange-50/30">{fmt(r.slTE + r.slBE)}</td>
                  {/* Closing HE */}
                  <td className={`px-1 py-1.5 text-right font-semibold bg-purple-50/30 ${r.clA < 0 ? 'text-red-700' : 'text-purple-800'}`}>{fmtN(r.clA)}</td>
                  <td className={`px-1 py-1.5 text-right font-semibold bg-purple-50/30 ${(r.clB + r.clC) < 0 ? 'text-red-700' : 'text-purple-800'}`}>{fmtN(r.clB + r.clC)}</td>
                  {/* Closing NHE */}
                  <td className={`px-1 py-1.5 text-right font-semibold bg-purple-50/30 ${r.clJE < 0 ? 'text-red-700' : 'text-purple-800'}`}>{fmtN(r.clJE)}</td>
                  <td className={`px-1 py-1.5 text-right font-semibold bg-purple-50/30 ${(r.clTE + r.clBE) < 0 ? 'text-red-700' : 'text-purple-800'}`}>{fmtN(r.clTE + r.clBE)}</td>
                </tr>
              ))}
              {stockRows.length === 0 && (
                <tr>
                  <td colSpan={22} className="text-center text-gray-400 py-8">No flocks found</td>
                </tr>
              )}
            </tbody>
            {stockRows.length > 1 && (
              <tfoot>
                <tr className="bg-gray-100 border-t-2 border-gray-300 font-semibold text-xs">
                  ...
                  {/* Opening */}
                  <td colSpan={2} className="px-1 py-2 text-right text-sky-800 bg-sky-50/30">{fmtN(totals.opHE)}</td>
                  <td className="px-1 py-2 text-right text-sky-800 bg-sky-50/30">{fmtN(totals.opHE)}</td>
                  <td className="px-1 py-2 text-right text-sky-800 bg-sky-100/30">{fmtN(totals.opNHE)}</td>
                  {/* Received */}
                  <td colSpan={2} className="px-1 py-2 text-right text-green-800 bg-green-50/30">{fmtN(totals.recHE)}</td>
                  <td colSpan={2} className="px-1 py-2 text-right text-green-800 bg-green-50/30">{fmtN(totals.recNHE)}</td>
                  <td className="px-1 py-2 text-right text-green-700 bg-green-100/30">{fmtN(totals.recLE)}</td>
                  {/* Wastage */}
                  <td colSpan={4} className="px-1 py-2 text-right text-red-700 bg-red-50/30">{fmtN(totals.wst)}</td>
                  {/* Sales */}
                  <td colSpan={2} className="px-1 py-2 text-right text-orange-800 bg-orange-50/30">{fmtN(totals.slHE)}</td>
                  <td colSpan={2} className="px-1 py-2 text-right text-orange-800 bg-orange-50/30">{fmtN(totals.slNHE)}</td>
                  {/* Closing */}
                  <td colSpan={2} className={`px-1 py-2 text-right bg-purple-50/30 ${totals.clHE < 0 ? 'text-red-700' : 'text-purple-900'}`}>{fmtN(totals.clHE)}</td>
                  <td colSpan={2} className={`px-1 py-2 text-right bg-purple-50/30 ${totals.clNHE < 0 ? 'text-red-700' : 'text-purple-900'}`}>{fmtN(totals.clNHE)}</td>
                </tr>
              </tfoot>
            )}
          </table>
