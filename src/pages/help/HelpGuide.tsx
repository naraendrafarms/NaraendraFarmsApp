...
const CHANGELOG: ChangeEntry[] = [
  { date: '2026-09-16', tag: 'New', text: 'VHL \u2192 BULK (SHED-WISE) DAILY ENTRY NOW HAS RECD \u2640 / RECD \u2642. Birds arriving into a shed \u2014 a transfer in from the rearing site, or a placement that comes in over several days rather than all at once \u2014 had nowhere to go on this screen. The only way to record them was to inflate Opening, which hides the fact that anything arrived and makes the day\u2019s numbers impossible to read back. The whole-flock Daily Entry screen has always had a Received field and the database has always had the columns; only this shed-wise grid was missing them. TWO NEW GREEN BOXES sit right after Open \u2642, and Closing now works out as OPENING + RECEIVED \u2212 TRANSFER \u2212 CULL \u2212 DEATH, recalculated as you type. Enter an arrival under Recd, NOT by raising Opening \u2014 the next day\u2019s Opening auto-fills from today\u2019s Closing, so what arrived today is already counted tomorrow and must not be entered twice. Template, Import and Export carry the two columns as well (Recd F, Recd M), so a spreadsheet round-trip matches what is on screen. NOTHING WAS CHANGED IN THE DATABASE and no existing row was touched \u2014 rows saved before today simply have Received of zero, which is what they meant.' },
  { date: '2026-09-16', tag: 'Fix', text: 'ASSIGN FLOCK TO SHEDS: YOU COULD NOT TELL WHICH SITE A SHED BELONGED TO. The shed list grouped sheds under a small site heading, but the heading scrolled away with the list \u2014 so once you had scrolled down you were ticking boxes that read only \u201cShed 7 (Shed 7)\u201d, and more than one site HAS a Shed 7. Ticking the wrong one puts a flock in the wrong site\u2019s shed, which then follows through into daily entry and every report that reads from it. THREE CHANGES, all to how the list is DISPLAYED \u2014 no flock, shed or assignment was altered: the SITE HEADING NOW STICKS to the top of the list while you scroll that site\u2019s sheds, so you can always see where you are; EVERY ROW NOW CARRIES ITS SITE CODE \u2014 \u201cBJP2 \u00b7 Shed 7\u201d \u2014 so a row read halfway down is never ambiguous, the same way the flock\u2019s own Shed Allocation tab already showed it; and SHEDS NOW SORT BY NUMBER rather than as text, so Shed 10 comes after Shed 9 instead of sitting between Shed 1 and Shed 2. The site heading also spells out the full site name beside its code.' },
  { date: '2026-09-15', tag: 'Improved', text: 'THE CHANGE HISTORY OLDER THAN 30 DAYS NOW LIVES IN A FILE, AND THE DATABASE IS HALF THE SIZE IT WAS. The audit log \u2014 the diary of who changed what, kept since 18/08/2026 \u2014 had grown to 584,603 entries and 240 MB, which was 82 PERCENT of the entire 291 MB database. All the real farm data put together, every flock, sale, salary, invoice, attendance and cash entry since June, is about 51 MB. The diary had become five times the size of the books it was describing, and the free plan stops at 500 MB. WHAT HAPPENED: 469,548 entries older than 16/08 were written to a compressed file and removed from the database. The DATABASE FELL FROM 291 MB TO 147 MB and the audit log from 240 MB to 96 MB \u2014 from 58 percent of the limit to 29 percent. NOTHING OF YOURS WAS TOUCHED, and this is the important part: not one flock, sale, salary, invoice, attendance or cash record moved or changed. Only the diary ABOUT changes was trimmed. Everything you look at day to day is exactly where it was. WHAT YOU WILL NOTICE: Admin Centre \u2192 Audit Log now shows the LAST 30 DAYS, and UNDO works on those \u2014 which is the only window anyone realistically undoes in. Older history is still yours: it sits in Supabase Storage under audit-archive as a dated file you can download, readable by admins alone. It is a file to open rather than a page to browse. HOW IT WAS MADE SAFE: the file was uploaded, then downloaded back and its fingerprint compared, and ONLY THEN were the rows deleted. Had any step failed, the job would have stopped with every row still in the database. FROM NOW ON it runs by itself on the 1st of each month, and only this first run was large \u2014 a normal month is about 30,000 entries. WHY IT GREW SO FAST: July and August alone added 550,000 entries, nearly all from bulk data imports, each of which writes one diary line per row loaded. Ordinary daily entry adds only a couple of MB a month.' },
  { date: '2026-09-15', tag: 'Fix', text: 'BILLS PAID IN CASH WERE ALL BEING CHARGED TO HO IMPREST, WHICHEVER SITE ACTUALLY PAID. Accounts \u2192 Pending Payments asks which BANK ACCOUNT a bill was paid from \u2014 one of your three \u2014 but only for NEFT, RTGS, IMPS, Cheque and UPI. Pick CASH and that box simply disappeared, and NOTHING replaced it. There was no way to say which imprest tin the money came out of, so the Cash Book row was written with no tin AND no site. The imprest rule is: the tin you tagged, else the site\u2019s own imprest, else HO Imprest \u2014 with the first two blank every cash bill payment fell through to HO IMPREST. The site tins never saw the spend, and HO Imprest carried spending it never did. Same fault as the NHE sales Head Office one fixed on 08/09: a form with no site field, a ledger row with no site, and a default that quietly says Head Office. NOW: choosing Cash shows PAID FROM IMPREST, listing your active tins with (HO) or (Site) beside each, and it is REQUIRED before the payment will save \u2014 on all three routes, the Pay button, BULK PAY and the edit form, since all three shared the same fault. The tin is stored on the bill the same way the bank account already was, so reopening a paid bill shows what you chose instead of losing it, and the Cash Book row now carries it, which is what puts the payment in the right Imprest Ledger. NOTHING ALREADY ENTERED WAS MOVED. Bills paid in cash before today still carry no tin and still show under HO Imprest \u2014 shifting them to the sites that really paid is a change to live records, so it waits on your say-so rather than happening as a side effect. Ask and it can be listed first: how many payments, how much, and which site each belongs to.' },
  { date: '2026-09-11', tag: 'New', text: 'SALARY RETURNED BY AN EMPLOYEE NOW HAS A PROPER PLACE. When a salary is paid to an employee\u2019s bank and they send it back \u2014 wrong account, paid twice, an overpayment recovered, someone who left \u2014 there was nowhere to record it except a manual Bank Ledger credit under \u201cOther\u201d, with nothing but the typed description tying it to the salary it reverses. A year later \u201cwhy did \u20b918,400 come in on 12/09?\u201d was answerable only by reading that description. THE NEW WAY: Accounts \u2192 Bank Ledger, add a CREDIT on the account that received it and pick the new category SALARY RETURN. Two boxes then appear \u2014 the employee, and which salary month came back \u2014 listing only salaries actually marked Paid, because an unpaid month cannot have been returned. Choosing the month fills the amount and a description for you the first time; change either freely, a PART return is fine. The SALARY REGISTER gains a RETURNED column beside Net Salary, with a total, and it is in the Excel export in the same position as on screen. WHY THE ORIGINAL PAYMENT IS LEFT ALONE, and this is the important part: the money really did leave the account and the return really did come back, so your bank statement has TWO lines and the ledger now has two as well. Do NOT instead set the salary back to Pending \u2014 doing that deletes the payment debit the app created, and if you have also entered the credit your bank balance reads HIGH by the returned amount with nothing on screen saying why. Move both sides or neither. If you pick Debit by mistake the form now tells you a salary return is money coming back IN. ONE THING UNDER THE BONNET worth knowing, because it protects you: the link is kept in a field of its own rather than on the existing salary-to-bank link. The salary form rebuilds that existing link from scratch every time a salary is saved, edited, reverted or deleted \u2014 it clears every bank row attached to that salary first \u2014 so a return recorded there would have been wiped by the next edit of that month, silently, leaving the bank balance wrong. On its own field nothing the salary form does can touch it. EMPLOYEE ADVANCES IS NOT THE PLACE for this: an advance is money going TO the employee and posts a debit, so recording a return there would send the money out a second time.' },
  { date: '2026-09-11', tag: 'Fix', text: 'PAYSLIPS WERE SHOWING ROUGHLY DOUBLE THE PF, BECAUSE THE PAYSLIP GENERATOR WAS STILL WORKING ON GROSS. PF was moved onto BASIC in the salary calculation, but the payslip never followed, and not because its 12 percent was wrong \u2014 because of what it called Basic. It filled its Basic box with the salary row\u2019s EARNED SALARY, which is the whole total earning: basic plus HRA plus allowance plus extra-days pay. Twelve percent of that is PF on gross. On a ₹20,000 salary split 10,000 basic / 6,000 HRA / 4,000 allowance, the register deducted ₹1,200 and the payslip printed ₹2,400. Three more faults came off the same line: the ₹15,000 RESTRICTED-PF CAP was never applied, so a capped employee\u2019s payslip overstated PF again on top of that; HRA was COUNTED TWICE in Gross Earnings, since it was already inside the figure in the Basic box and then added again \u2014 that example printed a gross of ₹26,000 against the register\u2019s ₹20,000; and ESI and Professional Tax, both taken off the same inflated figure, came out at ₹150 and ₹200 against the register\u2019s ₹75 and ₹150. Employer PF was a flat 12 percent as well, so it could never show the EPS and EPF-difference split the PF return is actually filed on. THE PAYSLIP NOW READS THE SALARY REGISTER RATHER THAN RECALCULATING IT. Basic, HRA, Allowance, Extra Days Pay, PF, ESI, PT, TDS, Advance and Other Deduction all come straight off the salary row, and the Auto ticks switch themselves off so nothing can overwrite them \u2014 which also means a figure you corrected by hand on the salary row now survives onto the payslip instead of being silently recomputed. The earnings lines are laid out to add up to the register\u2019s TOTAL EARNING exactly, with Allowance taken as the remainder of the row\u2019s gross so nothing can fall off the bottom, and EXTRA DAYS PAY now has its own line on the slip the way it has its own column in the register. Employer PF shows EPS plus the EPF difference as the register worked them out. WHEN NO SALARY HAS BEEN RUN for that month yet the slip still estimates, but now using the register\u2019s OWN rules \u2014 the same basic/HRA split, PF on basic with the cap, ESI on basic \u2014 so the two cannot disagree; Auto stays on in that case to show the figures are provisional. WHAT TO DO ABOUT PAYSLIPS ALREADY PRINTED: nothing changed in any salary record, and no payslip already saved was altered \u2014 the money deducted was always the register\u2019s figure. But any payslip HANDED TO AN EMPLOYEE showing PF on gross overstates what was taken from them, so those are worth reissuing: open the month in Payslip Generator and it will now print the register\u2019s figures.' },
  { date: '2026-09-09', tag: 'Fix', text: 'FLOCK 20: TWO HATCH BATCHES WERE ON THE WRONG INVOICE, AND ONE INVOICE HAD MORE EGGS SET AGAINST IT THAN IT EVER CARRIED. Yesterday\u2019s linking was reported as done because all 232 batches came away with a dispatch on them. That counted ROWS. It never checked the only thing that proves a link is right \u2014 that the eggs set against an invoice equal the eggs that invoice carried. Checked properly: 88 of the 90 Hitech dispatches tallied to the egg, and two did not. DC 4619 of 04/04/2026 (NF/HHF/26-27/4) carried 60,480 eggs but had 80,640 set against it, and DC 4620 of 06/04/2026 (NF/HHF/26-27/6) carried 60,480 and had only 40,320. The 80,640 is the one that mattered: more eggs set than an invoice carried is the nightly critical rule \u201cMore eggs set than the invoice carried\u201d, and it meant every hatch and fertility percentage on DC 4619 was measured against 20,160 eggs that never left the farm. THE CAUSE, AND IT WAS MINE: the rule laid both lists end to end in date order, and where several batches shared ONE setting date it put them in the order the rows happened to be stored in \u2014 which is random and means nothing. Three batches were set on 09/04/2026: Howrah 50,400, NilGanj 30,240 and NilGanj 10,080. The random order put the 50,400 first and it landed straight across the join between the two invoices. 89 of the 90 invoice boundaries fell cleanly between batches; that one did not, purely because of the order picked. NOW CORRECTED: the Howrah 50,400 of 09/04/2026 moves to DC 4620 and the NilGanj 30,240 moves to DC 4619. DC 4619 is now Ruiya Top Floor 30,240 + NilGanj 30,240 = 60,480, and DC 4620 is Howrah 50,400 + NilGanj 10,080 = 60,480. ALL 90 DISPATCHES NOW TALLY TO THE EGG and no invoice anywhere in the app is over-allocated. Only the link was written, on two rows \u2014 no eggs set, no setting date, no hatchery and no hatch result was touched, and the previous state is kept so it can be reversed exactly. WHAT TO TAKE FROM IT: \u201cevery row is linked\u201d is not the same as \u201cevery row is linked correctly\u201d, and Hatch Batches already shows you the difference \u2014 the Link Dispatch Invoice dropdown says how much of each invoice is already set and how much is left, so an invoice reading more set than it carried is visible on the form itself.' },
  { date: '2026-09-08', tag: 'New', text: 'FLOCK 20\u2019S HATCH BATCHES ARE NOW JOINED TO THE DISPATCHES THAT FED THEM. 232 batches held the hatch results you had typed in \u2014 hatchery, setting date, eggs set, fertile, chicks, hatchability \u2014 but only ONE was linked to a dispatch, so Hatch Analysis could not put a hatch % against the eggs that produced it. All 232 are now linked; none is left over and all 90 Hitech dispatches have their batches. WHY A RULE AND NOT A LOOKUP: the batches carry no DC and no invoice number, only a hatchery and a setting date, so there is nothing to join on. What made it safe is that the two sides reconcile EXACTLY \u2014 232 batches hold 53,42,409 eggs and the 90 dispatches sent 53,42,409, not one egg out. Both lists are laid end to end in date order and each batch takes the dispatch covering the midpoint of its running egg count, which is how a consignment set across several hatcheries over several days finds its way home. THE CHECK THAT IT WORKS: the one batch YOU linked by hand was left untouched, and the rule independently gives the same answer \u2014 Howrah, set 06/12/2025, 10,080 eggs, against DC 3861 dispatched 03/12/2025 on invoice NF/HHF/25-26/76. 88 of the 90 dispatches now tally to the egg with their batches. TWO DO NOT, and they are a pair: one carries 80,640 against 60,480 sent and the next 40,320 against 60,480 \u2014 corrected on 09/09/2026, see the entry below. NO HATCH RESULT WAS TOUCHED: only the link was written, and only where it was empty. Every previous link is kept so this can be undone exactly.' },
  { date: '2026-09-08', tag: 'Fix', text: 'OPENING A PAID EGG SALE COULD DESTROY ITS RECEIPT. Cash Received and Online were only ever SAVED on bird sales, so every egg sale reopened with both boxes showing 0 even when real money had come in \u2014 226 sales are in that state. Pressing Save from there read as \u201cthe payment was removed\u201d: the sale went back to Pending AND its cash book row was deleted, so the money vanished from the site\u2019s imprest, with nothing on screen to say it had happened. The split is now rebuilt when a sale is opened, from the cash book row itself, which knows exactly what came in as cash; an online-only receipt creates no such row, so its amount is restored to the Online box instead. Going forward both figures are saved on every sale, not just bird sales, and cleared properly when a payment really is removed. IF YOU EDITED AN EGG SALE TODAY AND ITS PAYMENT DISAPPEARED, that is what happened \u2014 re-enter the amount and it will stick.' },
  { date: '2026-09-08', tag: 'Improved', text: 'IMPREST LEDGER NOW OPENS ON TODAY, AND EMPTIED SHEDS STOP ASKING FOR ENTRIES. The ledger listed oldest first, so HO Imprest opened on April and today\u2019s entries were 930 rows down \u2014 which is why June and July receipts could not be found at all. It now shows NEWEST FIRST, with a link in the header to flip back. IMPORTANT, AND WHY IT WAS NOT SIMPLY REVERSED: the Balance column is the account\u2019s real balance ON EACH ENTRY, so it must still be worked out oldest to newest \u2014 only the finished list is turned round, after the balance and after any search. Every row keeps the balance it truly had on its own date, and Print still goes oldest first, the way a passbook reads. SEPARATELY, BULK DAILY ENTRY offered every shed a flock had EVER been in and never dropped one that emptied \u2014 Kethireddypally Shed 9 kept appearing after Flock 22 was sold and moved out of it on 06/09/2026, when it closed at 0 birds. A shed is now hidden once its last record BEFORE the date you are entering closed at zero. Deliberately \u201cbefore\u201d and not \u201con or before\u201d, so the emptying day itself can still be entered and back-dated, and a BROODING SHED STAYS AVAILABLE right up to the day its birds move to the grower shed \u2014 the grower shed appears the moment the transfer is recorded. 24 of 56 flock-shed pairs are currently empty and drop out.' },
  { date: '2026-09-08', tag: 'Fix', text: 'Rs 6,37,463 OF SITE RECEIPTS PUT BACK WHERE THEY WERE TAKEN. 17 cash receipts from 03/06/2026 to 22/07/2026 had lost their site through the Head Office fault fixed today, and a receipt with no site counts in HO IMPREST \u2014 so that money was sitting at Head Office instead of on the farm that took it. They are back with their flock\u2019s site: Agraharam Potlapally 11 (Rs 4,29,160), Bodjanampet-2 (VHL) 5 (Rs 2,08,108), Bodjanampet-1 1 (Rs 195). HO Imprest goes from Rs 11,90,998 to Rs 5,53,535 and those three site tins rise by the same amounts. No NHE receipt is left without a site. ONLY THE LOCATION WAS SET \u2014 the imprest is still worked out from it, exactly as an ordinary receipt is, because naming an account would claim a PERSON held the cash and no record says that. Every row\u2019s previous site is kept, so this can be reversed exactly.' },
  { date: '2026-09-08', tag: 'Fix', text: 'MONEY ALREADY TAKEN OUT OF WAGES WAS STILL BEING ASKED FOR. A sale to an employee settled by salary deduction kept showing on NHE Sales \u2192 Employee Dues even after the salary was paid \u2014 Rs 26,415 across 83 sales. THE PROOF IT WAS REALLY DEDUCTED, not an assumption: each payslip stores its own flock-deduction total, and for all 47 employee-months concerned that total equals what the app had already ticked off PLUS what it was still chasing \u2014 Rs 26,415 against Rs 26,415, to the rupee. So the money left the payslip; only the deduction records were never moved off \u201cpending\u201d, and a sale is only marked settled when they are. WHY IT HAPPENED: marking a salary Paid ticks off the deductions that are pending AT THAT MOMENT, and nothing re-checks afterwards \u2014 so a deduction raised later against a month already paid stayed pending for ever. The 83 are corrected and Employee Dues drops from Rs 62,915 to Rs 36,500, which is money genuinely still owed. AND IT CANNOT COME BACK: a database trigger now settles the sale whenever a deduction is taken, whatever caused it \u2014 the Paid button, an import, or a correction run \u2014 instead of depending on one button being pressed in one order. It uses the same rule the app uses (cash and online recorded, plus deductions genuinely taken), so the two can never disagree, and it can never block a save if it fails. NOTHING FINANCIAL MOVED: no cash book entry, no imprest balance and no salary figure changed, because the money had already left the payslip. Only how much shows as OWED changed. The previous state of every row is kept so this can be reversed exactly.' },
  { date: '2026-09-08', tag: 'Fix', text: 'NHE SALES ALWAYS SAID \u201cHEAD OFFICE\u201d FOR CASH RECEIVED AT, AND EDITING A SALE MOVED ITS CASH THERE. Opening any cash sale to change a rate or a DC number showed Head Office in Cash Received At whatever site you had originally chosen \u2014 and because saving deletes and rewrites the cash book entry, pressing Save then MOVED that cash off its site. THE CAUSE: nhe_sales has no cash_farm_id column at all. The location is written onto the CASH BOOK ROW, and the form was reading it back off the sale, where it never existed \u2014 so it read blank every time and fell back to Head Office, which then saves as a blank site. It now reads the location from the cash book entry the sale created, which is the only place it is kept. ALREADY AFFECTED: 17 cash receipts totalling Rs 6,37,463 are sitting with no site and therefore in HO Imprest \u2014 their flocks say Agraharam Potlapally (11, Rs 4,29,160), Bodjanampet-2 VHL (5, Rs 2,08,108) and Bodjanampet-1 (1, Rs 195). They have NOT been moved; that is a separate decision. ALSO FIXED IN THE SAME PLACE: a new sale now opens on the FLOCK\u2019S OWN SITE instead of Head Office, and follows the flock if you change it \u2014 unless you picked a location yourself, which is never overwritten. All three location boxes (the sales form, Receive Payment and Bulk Receipt) now open on \u201c\u2014 Select location \u2014\u201d and refuse to save blank, so a receipt genuinely taken at Head Office and one where nobody chose are no longer the same record. AND A RECEIPT THAT COULD NOT SAVE AT ALL: Receive Payment and Bulk Receipt were both sending cash_farm_id in their update to nhe_sales, so the database rejected the whole update and a cash receipt against an NHE sale failed. That field is gone from both. THE IMPREST BOX WAS NEVER BROKEN \u2014 it reads a column that does exist, and only showed \u201cHO Imprest (default)\u201d because that label is worked out from the location. With the location right, the imprest name follows. Also measured: of 299 cash sales, none has ever had an imprest picked by hand, so every one is still deriving from its location \u2014 which is why getting the location right was the whole job. THE SITE COLUMN in the sales list had the same fault and printed \u201cHead Office\u201d on every row; it now reads the cash book too, and shows a dash where no cash came in rather than pretending it went to Head Office.' },
  { date: '2026-09-08', tag: 'New', text: 'FLOCK 20\u2019S MISSING SIX MONTHS OF HATCHING-EGG SALES ARE IN. The app held Flock 20 HE only from June 2026, while the sheds had been laying since December \u2014 62.3 lakh eggs produced against 20.5 lakh dispatched, so 41.8 lakh eggs had been produced with no sale against them and every cost-per-egg and income figure for the flock was wrong. 72 Hitech Hatch Fresh dispatches from 03/12/2025 to 28/05/2026 are now loaded, with all 252 DAY-WISE LINES, so a dispatch that changed rate part way through keeps the right rate on each production date: 39,51,360 eggs, 77,434 free, 38,73,926 billable, Rs 12,83,40,373 billed, Rs 1,28,333 TDS at 0.1%. EVERY ONE OF THE 72 TIES TO THE INVOICE REGISTER TO THE RUPEE. Two things had to be settled first and both are worth knowing. FREE EGGS ARE 1.96% OF THE LOAD, not of each line: adding up the per-line free figures gives an egg or two too many on some dispatches, and the invoice is raised on total less that 1.96%, so a 60,480 load bills 59,295. AND ONE LINE WAS ON THE WRONG CHALLAN \u2014 514 eggs dated 26/04 sat on DC 4626, whose other three lines all went on 24/04; it belongs to DC 4627 of the same day. Left alone, invoice 18 over-billed by Rs 16,473 and invoice 20 under-billed by Rs 16,506; moved, they come to Rs 19,41,911.25 and Rs 22,65,546.75, which are the register\u2019s figures exactly. NOTE THESE ARE UNPAID AS FAR AS THE APP KNOWS: the workbook carried no receipt dates, so all 72 are marked Pending and Hitech now shows about Rs 12.82 crore outstanding. That is a gap in the data, not a debt \u2014 send the receipt dates and Bulk Receipt on Party Outstanding will settle them in one pass. The 16 non-Hitech lines in the same workbook (1,75,901 eggs, Meghana, Akshaya, Venkatadri, Jamal, Raju, Ellandula) were left out on purpose: their invoices cover other flocks too, so the sheet holds only part of each one and the amounts cannot be worked out from it.' },
  { date: '2026-09-05', tag: 'Fix', text: 'CASH BOOK \u2192 TRANSFER SAID \u201cCHOOSE BOTH FROM AND TO\u201d WHEN BOTH WERE ALREADY CHOSEN. The From and To boxes had no blank \u201c\u2014 Select \u2014\u201d option, and a dropdown whose stored value matches none of its options DISPLAYS THE FIRST ONE anyway \u2014 so both boxes showed an account name while the form actually held nothing, and saving failed. It only worked if you happened to change a box to a different account, which is why it seemed random. Both boxes now open on \u201c\u2014 Select source \u2014\u201d and \u201c\u2014 Select destination \u2014\u201d, so what you see and what the form holds are the same thing. The Imprest Ledger\u2019s transfer always had that option, which is why only the Cash Book was affected; every other dropdown on the page was checked and supplies its own blank entry. USE THIS FOR A BANK WITHDRAWAL: From = the bank, To = the imprest that received the cash. It writes both halves at once \u2014 a debit in the Bank Ledger and a receipt into that imprest \u2014 so drawn cash is never counted as income.' },
  { date: '2026-09-05', tag: 'New', text: 'SITE AND HEAD OFFICE EXPENSES NOW REACH THE FLOCKS THAT CAUSED THEM. Flock \u2192 Financial and Cost & Income counted only expenses TAGGED to a flock, and 413 of the 417 carry no flock \u2014 so every flock except 23 showed Rs 0 of other expenses while its site was plainly spending money. THE RULE, and it is the owner\u2019s own: a site\u2019s spending IS its flock\u2019s cost. Where one flock was at a site that month it carries 100% with no arithmetic; where two genuinely shared \u2014 measured as Kethireddypally only, in May/June 2025 and August/September 2026 \u2014 the site\u2019s spending splits between them by bird-days, the same basis site salary and electricity already use. Head Office, Feed Mill and anything else with no birds at its own site spreads across all flocks the same way. Each flock now shows an \u201cOf which\u201d line under its expenses: how much was booked straight to it, how much came from its site, and how much is its share of Head Office \u2014 and it says plainly whether any site was shared in the months concerned, so an apportioned figure is never mistaken for a whole bill. Every rupee lands: Rs 18,24,400 billed, Rs 18,24,400 allocated, all 417 expenses reaching a flock, none left over. Nothing is written to any expense \u2014 the split is worked out at read time, so it can be traced back and changes as soon as a bird count or a flock tag is corrected.' },
  { date: '2026-09-05', tag: 'Fix', text: 'DAILY FARM SUMMARY: A FLOCK SPLIT ACROSS TWO SITES NOW APPEARS UNDER BOTH. Flock 22 has birds at Kethireddypally AND Agraharam Potlapally, but the report showed it only under Kethireddypally \u2014 so Agraharam\u2019s page silently left out birds that are physically there. The cause: the report asked the FLOCK where it was, reading its single laying-farm field, when the truthful answer is on the daily records, where every row carries the shed and every shed carries its site. Measured before fixing: THREE OF THE FOUR flocks with records span two sites \u2014 Flock 19 (Kpally + Ppally), Flock 20 (Bpet1 + Kpally) and Flock 22 (Kpally + Ppally) \u2014 so this is the normal case, not an oddity. Each flock now gets ONE BLOCK PER SITE, listing only that site\u2019s own sheds, with its own bird counts, feed, eggs, mortality and manpower; the two blocks add up to the flock\u2019s full day. Yesterday\u2019s HD% for the +/- line is also compared site by site, since comparing one site\u2019s figure today against the flock\u2019s combined figure yesterday put a variance on the line that neither site had moved. Blocks are ordered by site then flock, the site filter now works on where the birds actually are, and the export carries the block\u2019s own site. A flock with no records for the day still appears once, under the site its master record names, so nothing disappears.' },
  { date: '2026-09-05', tag: 'New', text: 'IMPREST LEDGER NOW PRINTS, AND EXPORT IS ALWAYS VISIBLE. There was no Print at all, and the Export button only appeared once entries had loaded \u2014 so on an empty or slow view it looked as though neither existed. Both now sit permanently in the header, greyed out when there is nothing to send. Print opens the shared report layout, so the header, logo and footer match every other printed report and it saves to PDF from the browser dialog. The printed statement names the account and the period, and carries Opening and Closing \u2014 or, when a party/category search is on, says plainly that it is a FILTERED EXTRACT of N of M entries with the matching received and paid totals, so a page handed to someone else cannot be mistaken for the account\u2019s full ledger. Excel and Print send exactly the same rows, built from one shared definition so the two can never drift apart.' },
  { date: '2026-09-05', tag: 'New', text: 'IMPREST LEDGER: PARTY / DESCRIPTION SEARCH AND CATEGORY FILTER. The ledger had only an account and a date range, so a question like \u201chow much did the Creta cost out of this tin\u201d could not be answered there at all. Type a name in the search box \u2014 Creta, diesel, a person \u2014 and it matches the PARTY field AND the description, because a vehicle is often written into the description rather than the party box and a search that missed those would quietly under-report it. The Category box narrows to expense, salary, advance and the rest, using the same list as the Cash Book. IMPORTANT \u2014 HOW THE FIGURES BEHAVE WHEN A FILTER IS ON: Opening and Closing at the top keep describing the WHOLE account, and the Balance column keeps showing the account\u2019s real running balance at each entry, not a running total of the rows that matched. Filtering before the balance was worked out would produce a column that looks like a balance but belongs to no tin and reconciles with nothing. Instead an amber line appears showing \u201cMatching N of M entries\u201d with the Received and Paid totals for just those rows \u2014 that is your answer. Export follows the filter, so you get exactly the rows on screen. FOR VEHICLE SPEND ACROSS THE WHOLE BUSINESS, Purchase \u2192 Farm Expenses already has a Vendor dropdown listing every vendor ever used \u2014 that is the better screen when you do not care which tin paid, since a vehicle expense is recorded with the vehicle in the Vendor field.' },
  { date: '2026-09-05', tag: 'Improved', text: 'MANDAL IMPREST\u2019S 46 ENTRIES MOVED TO THE SITES THAT SPENT THE CASH. Mandal was carrying Rs 2,82,553 of payments that each belonged to a different site \u2014 Bpet1 bike petrol, Ppally diesel, Bpet2 DG usage \u2014 which pushed it to minus Rs 1,72,234 while those sites\u2019 own tins looked untouched. They now sit where the cash was actually spent: Agraharam Potlapally 17 entries (Rs 1,16,550), Bodjanampet-1 20 (Rs 1,16,490), Bodjanampet-2 (VHL) 4 (Rs 40,000), Feed Mill 3 (Rs 4,513), and the 2 Head Office entries (Rs 5,000) to HO Imprest, which is the fallback for any site without a tin of its own. Mandal is back to exactly its opening balance of Rs 1,10,319, holding what it started with. NOTHING ABOUT COST CHANGED: the site on every entry \u2014 which decides whose cost it is \u2014 was never touched, so no P&L, site expense or flock figure moves by a rupee. Only WHICH TIN THE CASH LEFT was corrected. SOME BALANCES ARE NOW NEGATIVE \u2014 Bodjanampet-2 at minus Rs 16,016 and Kethireddypally at minus Rs 2,40,181 \u2014 which is expected and accepted for now: those tins were funded by transfers that have not been entered yet, and the app only counts what it has been told. Entering those transfers, or the opening balances on Masters \u2192 Cash Imprest Accounts, brings them right. The previous tagging is kept in full so this can be reversed exactly if the split turns out to be wrong.' },
  { date: '2026-09-05', tag: 'New', text: 'ALL FLOCKS (DATA): SHEDS NOW IN ORDER, AND EVERY TAB EXPORTS AND PRINTS ITS OWN DATA. Daily Records listed sheds shuffled within a date \u2014 4, 2, 3, 1, 5 \u2014 because the list could only be sorted by columns of the daily record itself, and shed number lives on the linked shed row, so it fell back to sorting by the record\u2019s internal id. Rows now come newest date first, then site, then SHED IN ORDER, and 10 sorts after 9 rather than between 1 and 2 as a plain text sort would put it. Rows with no shed sit last, since those are the flock-level grade totals. SHED ALLOCATION had the same fault and is fixed the same way. EXPORT EXCEL AND PRINT / PDF now sit on EVERY tab \u2014 Overview, Daily Records, Shed Allocation, Transfers, HE Dispatch, Hatch Batches, Egg Conversions, Egg Sales (NHE), Feed, Medicine and Cull Sales \u2014 and each one exports ITS OWN columns, not a generic dump: the exact rows on screen, in the order on screen, honouring the filters above them. Excel is a real .xlsx workbook with a named sheet, not a CSV renamed, and Print opens the shared report layout so the header, logo and footer match every other printed report and it saves to PDF from the browser dialog. Daily Records exports EVERY filtered record rather than only the page you are looking at \u2014 an export that quietly stopped at 50 rows would be worse than none. The Feed tab\u2019s older CSV-only Export button is gone, replaced by the same pair as every other tab.' },
  { date: '2026-09-05', tag: 'New', text: 'FEED MILL NOW HAS ITS OWN IMPREST. When site imprests were first created they were given only to rearing and laying farms \u2014 the Feed Mill and Head Office were skipped. But the Feed Mill does hold and spend cash, so its entries belonged to NO cash box at all: they showed on Accounts \u2192 Imprest Ledger only under \u201c\u26a0 Not assigned to any imprest\u201d and counted in no balance anywhere. \u201cFeed Mill Site Imprest\u201d now exists and picked up 9 entries immediately \u2014 Rs 42,818 received and Rs 10,139 paid out, so it opens at Rs 32,679 rather than the negative figure a pure expense account would show. NO OPENING BALANCE WAS INVENTED: like every other imprest it starts at zero, so that Rs 32,679 is only what the app has recorded since, not what the tin actually holds. Set the real opening balance and date on Masters \u2192 Cash Imprest Accounts and the figure becomes true. HEAD OFFICE IS STILL UNASSIGNED and was deliberately left alone \u2014 134 entries worth about Rs 3.7 lakh remain under \u201cNot assigned\u201d, because Head Office cash could sit with HO Imprest, Mandal, or a named person and picking one without being told would put a false balance on a real person\u2019s account.' },
  { date: '2026-09-04', tag: 'New', text: 'ONE PAYMENT CAN NOW SETTLE SEVERAL VOUCHERS AT ONCE. A buyer or employee handing over one lump sum against five unpaid vouchers meant five separate trips through the Receive Payment window, re-typing the same date and re-picking the same imprest each time \u2014 and nothing stopped voucher 3 being tagged to a different tin than voucher 1. A "Receive" link now sits beside each name in the Employee Dues and Party (Buyer) Dues panels on NHE Sales, and a "Receive All" link on every party row in Reports \u2192 Party Outstanding. It lists every voucher that payer still owes on, oldest first, with a tick against each; enter ONE amount, ONE date, ONE cash location and ONE imprest, and the money is allocated down the list as you type \u2014 each row shows what it takes and what is still due, so you can see exactly where the payment lands before saving. The last voucher the money reaches is marked PARTIAL when it runs out and the rest stay fully due; nothing is spread evenly, because a part-paid voucher is a real state and five half-paid ones is a mess. Untick any voucher to leave it out. If the amount is more than the ticked vouchers can absorb, it says so and refuses to save rather than quietly parking the surplus somewhere. IMPORTANT \u2014 DO NOT USE IMPREST LEDGER \u2192 ADD VOUCHER FOR THIS: that writes a cash entry with no link back to the sales, so the vouchers stay Pending for ever AND the same cash is counted twice if they are later receipted properly. Add Voucher is for cash with no sale behind it. Works for hatching-egg dispatches and NHE sales together for a buyer, and for NHE sales for an employee. Each receipt is added as its own cash book row rather than replacing what was there, so a voucher paid in three instalments keeps three traceable entries.' },
  { date: '2026-09-04', tag: 'New', text: 'NHE SALES CAN NOW NAME THE IMPREST THAT TOOK THE CASH. Farm Expenses has had a "Paid from (Imprest)" box since last month; the SALES side had nothing, so a receipt could only say WHERE the cash came in, never WHO ended up holding it. Cash from a Bodjanampet-1 sale handed to Mandal, Naraendra or Srinath was still counted in Bodjanampet-1\u2019s tin \u2014 that balance read too high and the holder\u2019s too low, with nothing on screen to explain the difference. A "Received into (Imprest)" box now sits under Cash Received At on the NHE Sales form AND in the Receive Payment window, so a CREDIT SALE billed today and collected weeks later can name the right tin at the moment the money actually arrives. LEAVE IT BLANK for ordinary entry: blank means the location\u2019s own imprest, exactly what the app already worked out for itself, and the box shows that default by NAME (e.g. "Bodjanampet - 1 Site Imprest (default)") so nobody has to know the rule to trust it. Fill it in only when a person collected on a site\u2019s behalf. It is saved on the sale as well as the cash book entry, so reopening a sale shows what was chosen instead of resetting. A sale with NO cash \u2014 fully online, or still unpaid \u2014 is never tagged to an imprest, because a tin\u2019s name has no business on money it never held. Receive Payment is shared with HE Dispatch, so hatching-egg receipts get the same box; nothing already recorded is changed.' },
...
const SECTIONS: Section[] = [
    ...
    workflows: [
      ...
      },
      {
        ...
        steps: [
          ...
          { text: 'Opening is "birds already in that shed at the start of the day". Recd ♀ / Recd ♂ (the green pair right after Opening) is "birds that arrived into that shed during the day" — a transfer in from the rearing site, or a staged placement that comes in over several days.', note: 'Enter the arrival under Recd, not by inflating Opening — that way the day shows what came in, and the bird count still adds up. Closing = Opening + Received − Transfer − Cull − Death, worked out as you type.' },
          { text: 'On the next day, that shed\'s Opening auto-fills from today\'s Closing — so yesterday\'s Received is already inside it and must not be entered again.' },
  ...
  // ── REPORTS ───────────────────────────────────────────────────────────────────
  {
    ...
    workflows: [
      ...
      {
        title: 'Finding what a vehicle or party cost out of one imprest',
        path: 'Accounts → Imprest Ledger → Party / Description search',
        steps: [
          { text: 'Pick the imprest account, then type the name in the Party / Description search — e.g. Creta. It matches both the Party field and the description.' },
          { text: 'Add a Category (expense, salary, advance…) to narrow further, and a From/To date range if you want one period.' },
          { text: 'The amber line shows "Matching N of M entries" with Received and Paid for just those rows. That total is your answer.' },
          { text: 'Export gives exactly the rows on screen.',
            note: 'Opening and Closing at the top still describe the WHOLE account, and the Balance column is the account\u2019s real running balance at each entry — not a running total of the filtered rows. That is deliberate: a balance of only the matching rows would reconcile with nothing.' },
          { text: 'For vehicle spend across the whole business, use Purchase → Farm Expenses → Vendor instead — that covers every site and tin at once.' },
        ]
      },
      {
        title: 'One payment covering several vouchers',
        path: 'NHE Sales → Employee / Party Dues → Receive  ·  or  Reports → Party Outstanding → Receive All',
        steps: [
          { text: 'Open the dues panel at the top of NHE Sales (Employee Dues or Party Dues) and click Receive beside the name. For an outside buyer you can also use Receive All on Reports → Party Outstanding.' },
          { text: 'Every voucher that payer still owes on is listed, oldest first, each already ticked. Untick any you are not settling.' },
          { text: 'Enter ONE amount, ONE date, ONE cash location and ONE imprest. The money is allocated down the list as you type — each row shows what it takes and what remains due.' },
          { text: 'The last voucher the money reaches is marked Partial; the rest stay fully due. Save when the allocation looks right.',
            note: 'Nothing is spread evenly across vouchers. Oldest-first is how the outstanding list is already read.' },
          { text: 'For a buyer this covers HE dispatches and NHE sales together. For an employee it covers their NHE sales.',
            warning: 'Never enter a lump sum through Imprest Ledger → Add Voucher. That writes a cash entry with no link to the sales, so they stay Pending for ever and the cash is counted twice once they are receipted properly. Add Voucher is only for cash with no sale behind it.' },
        ]
      },
      {
        title: 'Recording who holds the cash from a sale',
        path: 'Flocks → NHE Sales → Received into (Imprest)',
        steps: [
          { text: 'Enter the sale as usual. When you type a Cash Received amount, two boxes appear: Cash Received At (Location) and Received into (Imprest).' },
          { text: 'Leave the Imprest box BLANK for normal entry — blank means that location\u2019s own imprest, and the box names the default so you can see where it will go.',
            note: 'Blank is not "unassigned". It is the site\u2019s own tin, which is what the app already derived on its own.' },
          { text: 'Fill it in only when a PERSON collected the cash on a site\u2019s behalf — Mandal, Naraendra or Srinath. That is the one thing the location cannot tell you.' },
          { text: 'For a credit sale, the same box appears in the Receive Payment window, so the tin is named on the day the money actually arrives, not the day the sale was billed.' },
          { text: 'A sale paid fully online, or not yet paid, is never tagged to an imprest.',
            warning: 'Getting this wrong does not lose money, but it misstates two balances at once — the site reads high and the holder reads low.' },
        ]
      },
      {
