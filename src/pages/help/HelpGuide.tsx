...
const CHANGELOG: ChangeEntry[] = [
  { date: '2026-08-19', tag: 'New', text: 'FLOCK MANAGEMENT \u2192 CHICK RECEIPTS: what was actually received, flock by flock, on one page \u2014 breeder females and males split into BILLED and FREE, the placed total, the supplier, the invoice number and date, the rate and the cost, with column totals across every flock. The figures were always in the database but there was nowhere to read them: billed and free sat in a HOVER TOOLTIP on the flock cards (invisible on a phone), as a small grey caption on the flock page, and the invoice number only on the edit form, so anyone asking \u201chow many breeder females did we get, and on which invoice\u201d had to open one flock at a time. The page also does two checks nobody was doing by hand. Where a flock has only a placed total and no billed/free split, it says so and lists the flocks \u2014 a dash there means the figure was never entered, not that it was zero. And where billed plus free does NOT equal the placed figure, the row is highlighted rather than quietly corrected, because that difference is either a typo or a delivery nobody recorded. Exports to CSV.' },
