...
const CHANGELOG: ChangeEntry[] = [
  { date: '2026-09-02', tag: 'New', text: 'MASTERS → LINE MASTER. The cage lines inside each shed now have a screen of their own: side (A–D), line number, boxes, and bird capacity female/male, grouped by shed with a running total. Each shed total is shown against the figure already held in the Sheds master and flagged if the two disagree, so a mistyped box count shows up immediately rather than months later. Loaded so far: Kethireddypally (12 sheds, 292 lines, 24,044 boxes — agreeing with the shed master on every one of the 12) and Agraharam Potlapally (4 sheds, 192 lines, capacity birds). Bodjanampet-1 and Bodjanampet-2 are deliberately NOT loaded: one sheet did not add up to its own printed totals and the other was reported as containing a mistake. Every row is marked Provisional until confirmed. Deleting a line warns that its production, mortality and feed history goes with it.' },
