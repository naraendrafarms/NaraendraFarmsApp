...
const CHANGELOG: ChangeEntry[] = [
  ...
  { date: '2026-07-18', tag: 'Fix', text: 'Accounts → Pending Payments: editing a bill that came from a GRN linked to a Purchase Order never showed the PO Number, even after linking — the database trigger that creates the Pending Payments row from a GRN never wrote it, in any version, going back to before the PO link even existed. Now correctly pulled from the linked PO and backfilled onto existing bills. Credit Days was already working correctly (it only ever comes from the vendor\'s own Credit Days in Parties Master, not from the PO).' },
  { date: '2026-07-18', tag: 'Fix', text: 'Purchase → GRN → Add/Edit GRN: typing a different Basic Amount or Total Amount than the auto-calculated qty×rate figure was silently discarded on save — the calculated value always won. Now respects whichever amount you actually typed into those two fields this session; changing qty/rate/GST%/other charges again resets back to following the live calculation.' },
  { date: '2026-07-18', tag: 'Improved', text: 'Purchase → GRN log: added a proper page-size selector (25/50/75/100 rows) with Prev/Next, instead of everything rendering onto one long page.' },
