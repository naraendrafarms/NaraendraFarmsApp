...
export const BankLedgerPage: React.FC = () => {
  ...
  // without closing the modal / scrolling back up to the top Add button
  // each time. Cleared whenever the modal is freshly opened via openAdd.
  const [savingAndNext, setSavingAndNext] = useState(false)
  const [addedThisSession, setAddedThisSession] = useState<{ date: string; amount: number; txn_type: string }[]>([])
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      // the user noticed and switched the FY dropdown themselves — auto-
      // switch it to match whatever was just entered.
      const savedFy = fyOfDate(form.txn_date)
      if (savedFy !== fy) setFy(savedFy)
      if (!editId) setAddedThisSession(prev => [...prev, { date: form.txn_date, amount, txn_type: form.txn_type }])
      qc.invalidateQueries({ queryKey: ['bank_transactions', selectedAccount] })
      ...
      if (savingAndNext && !editId) {
        // Keep the modal open, same date/account/type — just clear the
        // amount/description/links so the next entry starts fresh.
        setForm(f => ({
          ...EMPTY_FORM,
        ...
        <div className="space-y-4">
          ...
          {addedThisSession.length > 0 && (
            <div className="bg-green-50 border border-green-200 rounded-lg px-3 py-2 space-y-1">
              <p className="text-xs font-semibold text-green-800">Added this session ({addedThisSession.length})</p>
              {addedThisSession.map((a, i) => (
                <p key={i} className="text-xs text-green-700">
                  {a.date} — {a.txn_type} {inr(a.amount)}
                </p>
              ))}
            </div>
          )}
          <div className="flex justify-end gap-3 pt-2">
            ...
            {!editId && (
              <Button variant="outline" loading={saving && savingAndNext}
                onClick={() => { setSavingAndNext(true); handleSubmit() }}>
                Save &amp; Add Another
              </Button>
            )}
            <Button loading={saving && !savingAndNext}
              onClick={() => { setSavingAndNext(false); handleSubmit() }}>
              Save
            </Button>
          </div>
        </div>
