...
// Mirrors BuyerAdvancesPage.tsx exactly, but for the opposite direction —
// money paid OUT to a supplier ahead of a bill, recoverable against future
// GRN / Pending Payments bills (via the "Advance" payment mode there).
export const VendorAdvancesPage: React.FC = () => {
  ...
  const { data: parties = [] } = useQuery({
    queryKey: ['parties_suppliers_adv'],
    queryFn: async () => {
  ...
  const { data: advances = [], isLoading } = useQuery({
    queryKey: ['vendor_advances', filterParty],
    queryFn: async () => {
      let q = supabase
  ...
  const addMut = useMutation({
    mutationFn: async (row: typeof EMPTY & { bank_account_id?: string }) => {
      ...
      const { data: adv, error } = await supabase.from('vendor_advances').insert({
  ...
  const editMut = useMutation({
    mutationFn: async (row: typeof EMPTY & { id: string }) => {
      ...
      await supabase.from('cash_book').delete().eq('vendor_advance_id', row.id)
      await supabase.from('bank_transactions').delete().eq('vendor_advance_id', row.id)
      const partyName = (parties as any[]).find((p: any) => p.id === row.party_id)?.name ?? ''
      const narration = `Advance to ${partyName}${row.reference_no ? ` (${row.reference_no})` : ''}`
      const isCash = ['cash'].includes(row.payment_mode)
      if (isCash) {
  ...
  return (
    <div className="p-4 max-w-5xl mx-auto space-y-4">
      <SectionHeader
        ...
        subtitle="Record advances paid to suppliers — recoverable against future GRN / bill payments"
        action={
      ...
      <div className="grid grid-cols-3 gap-3">
        <Card className="p-4 text-center">
          ...
          <div className="text-lg font-bold text-red-700">{inr(totalPaid)}</div>
        </Card>
        <Card className="p-4 text-center">
          <div className="text-xs text-gray-500 mb-1">Used / Adjusted</div>
          <div className="text-lg font-bold text-gray-700">{inr(totalUsed)}</div>
        </Card>
        <Card className="p-4 text-center">
      ...
      <Card>
        {isLoading ? <Spinner /> : advances.length === 0 ? (
          <EmptyState title="No vendor advances recorded yet" />
        ) : (
          <Table>
            <thead>
              <tr>
      ...
      <Modal open={showModal} onClose={() => setShowModal(false)} title={editId ? 'Edit Vendor Advance' : 'Add Vendor Advance'}>
        <div className="space-y-4 p-1">
          ...
          <div className="flex justify-end gap-2 pt-2">
            ...
            <Button
              onClick={async () => {
                if (!form.party_id) return toast.error('Select a vendor')
