-- Migration 064: Fix audit log trigger table names and re-attach all triggers
-- grn_entries (not 'grn') is the correct table name

-- Re-create trigger function (idempotent)
...
-- Re-attach all triggers (now with correct grn_entries name + daily_feed)
DO $$
DECLARE
  t TEXT;
  tables TEXT[] := ARRAY[
    'daily_records','flock_transfers','nhe_sales','salary_monthly',
    'attendance_daily','grn_entries','electricity_bills','flocks','employees',
