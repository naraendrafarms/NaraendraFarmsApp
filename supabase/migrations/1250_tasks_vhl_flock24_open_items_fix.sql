-- Re-run of 1249, whose INSERT was rejected in full by tasks_priority_check:
-- priority is one of low, normal, high, urgent - there is no 'medium'. The
-- verify SELECT in 1249 returned 0, so NEITHER task was written; nothing to
-- clean up, only to insert. Same guard, so this cannot duplicate.
