SELECT string_agg(
  'F' || fl.flock_no || ' ' || fm.name || ' sh' || s.shed_no || ' ' || to_char(d.record_date,'YYYY-MM-DD') ||
  ': open(' || d.opening_female || '/' || d.opening_male || ')' ||
  ' trcull(' || d.trcull_female || '/' || d.trcull_male || ')' ||
  ' mort(' || d.mortality_female || '/' || d.mortality_male || ')' ||
  ' stored_close(' || d.closing_female || '/' || d.closing_male || ')' ||
  ' correct_close(' || (d.opening_female - d.trcull_female - d.mortality_female) || '/' || (d.opening_male - d.trcull_male - d.mortality_male) || ')',
  ' | ' ORDER BY fl.flock_no, fm.name, s.shed_no::int, d.record_date
) AS rows
