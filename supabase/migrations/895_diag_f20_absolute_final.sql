-- Migration 895 (READ ONLY): the absolute final state check for Flock 20.
SELECT 'f20_absolute_final' AS chk, count(*)::int AS n, sum(total_eggs)::bigint AS eggs_sum
  FROM public.daily_records WHERE remarks = 'F20_IMPORT_2026-08-24';

SELECT 'f20_absolute_mismatches' AS chk, count(*)::int AS mismatches
...
SELECT 'f20_absolute_dupes' AS chk, count(*)::int AS n
  FROM (
