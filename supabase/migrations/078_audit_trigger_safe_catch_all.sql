-- Migration 078: Make audit trigger safe — wrap in catch-all exception handler.
-- Audit log failures must NEVER block real INSERT/UPDATE/DELETE operations.
-- Also re-attaches trigger on cash_book to guarantee the latest function is used.
...
DECLARE
  ...
  v_summary    TEXT;
BEGIN
  -- Wrap EVERYTHING in exception handler so audit can never break real operations
  BEGIN
    BEGIN
      v_user_id    := auth.uid();
  ...
  EXCEPTION WHEN OTHERS THEN
    -- Audit log failure must never block the actual data operation
    NULL;
  END;
...
-- Re-attach trigger on cash_book to ensure it uses the updated function
DROP TRIGGER IF EXISTS trg_audit ON public.cash_book;
