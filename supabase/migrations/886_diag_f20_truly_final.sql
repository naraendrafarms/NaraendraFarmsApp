-- Migration 886 (READ ONLY): the real final state after migration 885.
SELECT 'f20_truly_final_count' AS chk, count(*)::int AS n, sum(total_eggs)::bigint AS eggs_sum
  FROM public.daily_records WHERE remarks = 'F20_IMPORT_2026-08-24';

SELECT 'f20_truly_final_mismatches' AS chk, count(*)::int AS mismatches
...
SELECT 'f20_dup_check_final' AS chk, count(*)::int AS n
  FROM (
