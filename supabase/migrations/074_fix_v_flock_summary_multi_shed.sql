-- Migration 074: Fix v_flock_summary to SUM closing birds across all sheds for latest date
-- Previously LIMIT 1 returned only one shed's record, giving wrong current_female/male for multi-shed flocks
...
SELECT
  ...
  -- Latest record date + SUM closing birds across ALL sheds for that date
