-- Retry of 691, which reported "Errors: 21" while the workflow still showed
-- green. Cause: the function body was quoted with a $fn$ marker, and run_sql.py
-- only recognises the plain double-dollar marker -- so it split the body on its
-- internal semicolons and fed the pieces to the database as separate
-- statements. Same function, quoted the way the runner understands.
--
