-- Retry of 705 as well as 704:
-- 705 ran clean (Errors: 0) but printed no result rows at all, so its findings
-- were captured and thrown away. This keeps the diagnostics table so a separate
...
-- Table deliberately KEPT: the runner printed nothing for the select above,
-- so the findings are read back by the next migration instead.
