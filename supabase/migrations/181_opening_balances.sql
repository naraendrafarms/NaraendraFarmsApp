-- Migration 181: Financial-year opening balances for parties (buyers/suppliers) and partners.
-- Dr = they owe us (receivable, e.g. buyer);  Cr = we owe them (payable, e.g. supplier/partner).
-- Party opening balances appear as the first row of v_party_ledger.

CREATE TABLE IF NOT EXISTS public.opening_balances (
  id          UUID DEFAULT uuid_generate_v4() PRIMARY KEY,
  fy          TEXT NOT NULL,                 -- e.g. '2026-27'
  as_of_date  DATE NOT NULL,                 -- e.g. 2026-04-01
  party_id    UUID REFERENCES public.parties(id)  ON DELETE CASCADE,
  partner_id  UUID REFERENCES public.partners(id) ON DELETE CASCADE,
  amount      NUMERIC(14,2) NOT NULL DEFAULT 0,
  dr_cr       TEXT NOT NULL DEFAULT 'Dr',    -- 'Dr' (receivable) | 'Cr' (payable)
  remarks     TEXT,
...
CREATE POLICY ob_select ON public.opening_balances FOR SELECT USING (true);
DROP POLICY IF EXISTS ob_insert ON public.opening_balances;
CREATE POLICY ob_insert ON public.opening_balances FOR INSERT WITH CHECK (true);
DROP POLICY IF EXISTS ob_update ON public.opening_balances;
CREATE POLICY ob_update ON public.opening_balances FOR UPDATE USING (true);
DROP POLICY IF EXISTS ob_delete ON public.opening_balances;
CREATE POLICY ob_delete ON public.opening_balances FOR DELETE USING (true);

-- Recreate v_party_ledger to include party opening balances (first row by date)
DROP VIEW IF EXISTS public.v_party_ledger;
CREATE VIEW public.v_party_ledger AS
  -- Opening balance (party)
...
SELECT 'opening_balances' AS tbl, COUNT(*) AS rows FROM public.opening_balances;
